...
os.environ["token"] = "1782077594:AAHdDWQxv_wAyVJP8zTSj591v1XgdkwFl4M"
os.environ["db_url"] = "mongodb+srv://ghaazzz:ghaazzzpass123%24@cluster0.oa5wk.mongodb.net/ghaazzzbot?retryWrites=true&w=majority"
...
def remind_game(update, context):
    ...
    if y - now - first_rem > 0:
        ...
        for mellat in db.users.find({"is_admin": 1,  "notif.2" : 1}):
            chat_id = mellat["tg_user"]["id"]
            last_message[chat_id] = context.bot.send_message(
                chat_id = chat_id,
                text = "۶ ساعت تا شروع بازی"
            )
    now = int(time.time())
    if y - now - second_rem > 0:
        time.sleep(y - now - second_rem)
        for mellat in db.users.find({"is_admin": 1, "notif.3" : 1}):
            chat_id = mellat["tg_user"]["id"]
            last_message[chat_id] = context.bot.send_message(
                chat_id = chat_id,
                text = "یه ربع تا شروع بازی"
            )
    now = int(time.time())
    if y - now > 0:
        time.sleep(y - now)
        for mellat in db.users.find({"is_admin": 1, "notif.4" : 1}):
            chat_id = mellat["tg_user"]["id"]
            last_message[chat_id] = context.bot.send_message(
                chat_id = chat_id,
                text = "بازی شروع شد"
